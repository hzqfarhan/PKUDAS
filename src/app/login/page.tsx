...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    // Simulate a brief delay for realism in mock mode
      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="rounded-xl border border-border bg-card p-6 shadow-sm space-y-4">
          ...
          <button
            ...
            disabled={loading || googleLoading}
            className="w-full relative flex items-center justify-center gap-2 rounded-lg border border-border bg-background py-2.5 text-sm font-semibold text-foreground hover:bg-muted transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
            {googleLoading ? (
